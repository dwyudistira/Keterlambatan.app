...
<div class="container mt-3">
    ...
    <div class="d-flex">
        <div class="p-2">
          <a href="{{ route('Ps.role.export.excel.lates') }}" class="btn btn-info " style="color: white;" >Export Data Keterlambatan</a>
        </div>
    </div>
      <table class="table ">
        <thead>
          <tr>
            <ul class="nav nav-tabs" style="margin-top: 10px">
                <li class="nav-item">
                  <a class="nav-link  " aria-current="page" href="{{ route('Ps.role.dataLate') }}" >Keseluruhan Data</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link active" aria-current="page" href="{{ route('Ps.role.dataRekap') }}}">Rekapitulasi Data </a>
                </li>
            </ul> 
          </tr>
                ...
                @if (!in_array($late->student->name, $uniqueNames))
                    ...
                    <tr>
                        ...
                        <td>
                          ...
                          <button type="button" class="btn"><a href="{{ route('Ps.role.dataLihat', $late->student_id) }}" style="text-decoration: none; color: black;">Lihat</a></button>
                            @if ($countNames[$late->student->name] >= 3)
                                    @csrf
                                    <button type="submit" class="btn btn-primary"><a href="{{ route('Ps.role.print', $late->id) }}" style="color: white; text-decoration: none;">Cetak Surat Pernyataan</a></button>
                                </form>
                            @else
